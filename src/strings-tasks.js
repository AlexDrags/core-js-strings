...
function countVowels(str) {
  ...
  for (let index = 0; index < str.split('').length; index += 1) {
    ...
    if (arrVowels.includes(element)) counter += 1;
  }
  return counter;
}
...
function isPalindrome(str) {
  ...
  let str2 = str.split(' ').join('').split(',').join('').toLowerCase();
  let bool = true;
  ...
  for (let index = 0; index < str1.length; index += 1) {
    ...
    if (element === str2[str2.length - 1 - index]) {
      bool = true;
    }
    if (element !== str2[str2.length - 1 - index]) {
      bool = false;
...
function findLongestWord(sentence) {
  const arr = [''];
  for (let index = 0; index < sentence.split(' ').length; index += 1) {
    const element = sentence.split(' ')[index];
...
function encodeToRot13(str) {
  const notRot13Str = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  ...
  for (let index = 0; index < str.split('').length; index += 1) {
    ...
    if (element === ' ') {
      newStr += ' ';
    } else if (notRot13Str.includes(element)) {
      newStr += rot13Str[notRot13Str.indexOf(element)];
    } else {
      newStr += element;
    }
  }
